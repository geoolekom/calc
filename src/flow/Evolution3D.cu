...
Evolution3D::Evolution3D(double tStep, State3D *state, Grid3D *grid, GeometryInterface *geometry, DoduladCI *ci)
...
State3D *Evolution3D::getState() const { return this->curr; };

void Evolution3D::swap() { std::swap(curr, prev); }
...
__device__ double Evolution3D::schemeChange(const intVector &xIndex, int vxIndex, int vyIndex, int vzIndex) {
    ...
    return prev->getValue(xIndex, vIndex) -
           gammaX * (limitValue(gammaX, {1, 0, 0}, xIndex + intVector({1, 0, 0}), vIndex) -
                     limitValue(gammaX, {1, 0, 0}, xIndex, vIndex)) -
           gammaY * (limitValue(gammaY, {0, 1, 0}, xIndex + intVector({0, 1, 0}), vIndex) -
                     limitValue(gammaY, {0, 1, 0}, xIndex, vIndex));
};
    ...
    for (int xzIndex = tzIndex; xzIndex < prev->zIndexMax; xzIndex += tzStep) {
        for (int xyIndex = tyIndex; xyIndex < prev->yIndexMax; xyIndex += tyStep) {
            for (int xxIndex = txIndex; xxIndex < prev->xIndexMax; xxIndex += txStep) {
                    ...
                    for (int vyIndex = prev->vyIndexMin; vyIndex < prev->vyIndexMax; vyIndex++) {
                        for (int vxIndex = prev->vxIndexMin; vxIndex < prev->vxIndexMax; vxIndex++) {
                            ...
                            if (grid->inBounds(v) && geometry->isFreeFlow(xIndex, v)) {
                                ...
                                if (value < 0) {
                                    printf("Ошибка: (%d, %d, %d), (%d, %d, %d)\t%.20f\n", xxIndex, xyIndex, xzIndex,
                                           vxIndex, vyIndex, vzIndex, value);
                                    value = prev->getValue(xIndex, vIndex);
                                }
                ...
                isMirrored = geometry->isMirrorReflection(xIndex, doubleDirection) ||
                             geometry->isMirrorReflection(xIndex, -1 * doubleDirection);
                isBorder = geometry->isBorderReached(xIndex, doubleDirection) ||
                           geometry->isBorderReached(xIndex, -1 * doubleDirection);
